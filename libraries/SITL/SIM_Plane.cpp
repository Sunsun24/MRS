...
void Plane::calculate_forces(const struct sitl_input &input, Vector3f &rot_accel)
{
    ...
    //**********after tens seconds when the aircraft is stable, change the mass and thrust_scale, the initial mass = 2.0
	uint32_t tnow = AP_HAL::millis();	
	uint32_t static n = 0;

    //printf("1111111111111111111111111123  mass = %lf\n",mass);  //*******************
	if(tnow > 115000  && n == 0)
	{
	    mass = 2.0f;
	    thrust_scale = (mass * GRAVITY_MSS) / hover_throttle;	
		n++;  
		//printf("1111111111111111111111111124  mass = %lf\n",mass);  //**************
	}
    // if(tnow > 130000  && n == 1)
	// {
	// 	mass = 1.0f;
	// 	thrust_scale = (mass * GRAVITY_MSS) / hover_throttle;	
	// 	n++;
	// 	printf("1111111111111111111111111111  n = %d\n",n);
	// }
