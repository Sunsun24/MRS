...
void Aircraft::update_position(void)
{
    ...
    // // calculate the distance between home and aircraft in meters    when  it   is   stable after tens seconds
    uint32_t tnow = AP_HAL::millis();
	static uint32_t i = 0;
	static uint32_t flag = 0;
	static uint32_t freq = 0;
	const uint32_t num = 10000;
	static Vector3f position_collect[num];
	uint32_t k = 0;
	...
	if(flag == 0)
	{
	    ...
	    if( freq == 25)
	    {
			...
			if(i<num)
		    {
		    	position_collect[i].x = position.x;
				position_collect[i].y = position.y;
				position_collect[i].z = position.z;
			    i++;
		    }
			else
			{
				printf("the number of position is out of %d\n", num);
			}
	    }
	}
	if(tnow > 150000)
	{
		...
		{
			flag++;
			printf("x = [...] and total  number is %d\n",i);
			for(k=0;k<i;k++)
			{
				printf("%f ",position_collect[k].x);
			}
			printf("\n");
			printf("y = [...]\n");
			for(k=0;k<i;k++)
			{
				printf("%f ",position_collect[k].y);
			}
			printf("\n");
			printf("z = [...]\n");
			for(k=0;k<i;k++)
			{
				printf("%f ",position_collect[k].z);
			}
			printf("\n");
		}
	}
...
void Aircraft::update_dynamics(const Vector3f &rot_accel)
{
	...
	// static uint32_t freq = 0;
	// const uint32_t num = 10000;
	// static float airspeed_collect[num];
	// uint32_t k = 0;
	...
	// 		if(i<num)
	// 	    {
	// 	    	airspeed_collect[i] = airspeed;
	// 		    i++;
	// 	    }
	// 		else
	// 		{
	// 			printf("the number of airspeed is out of %d\n", num);
	// 		}
	//     }
	// }
	// if(tnow > 150000)
	...
	// 	{
	// 		flag++;
	// 		printf("airspeed = [...] and total  number is %d\n",i);
	// 		for(k=0;k<i;k++)
	// 		{
	// 			printf("%f ",airspeed_collect[k]);
	// 		}
	// 		printf("\n");
	// 	}
	// }
...
//printf("1wwwwwwwwwwwwwwwwwwwwwwww  wind direction= %f,turbulence= %f,dir_z= %f,speed= %f,\n",input.wind.direction, input.wind.turbulence,input.wind.dir_z,input.wind.speed);
...
{
    external_payload_mass = 2.0f;  //***************0
    uint32_t tnow = AP_HAL::millis();  //*************
    static uint32_t num = 0;							
	#if 1
	char* buffer; buffer = getcwd(NULL,0); uint32_t len = strlen(buffer);  uint32_t sysid = 0;
	...
	if(tnow > 115000 )
	{
		...
		if (num == 0)
		{
			printf("1111111111111111111111111111  external_payload_mass = %f  sysid %d\n",external_payload_mass, sysid);
			num++;
		}
	}
	// if(tnow > 130000 )
	// {
	// 	external_payload_mass = 2.0f;
	// 	if (num == 1)
	// 	{
	// 		printf("2222222222222222222222222222  external_payload_mass = %f  sysid %d\n",external_payload_mass, sysid);
	// 		num++;
	// 	}
	// }
//**************************
