...
int32_t AP_YawController::get_servo_out(float scaler, bool disable_integrator)
{
	...
	//	float PID =  constrain_float(_last_out, -45, 45);
		float adpative_robust_value = _update_yaw_adaptive_robust_rule(_last_out, _pid_info.FF, _pid_info.P, _pid_info.D, _pid_info.I, delta_time);
		if (_last_out < -45)
		{
 ...
 */
float AP_YawController::_update_yaw_adaptive_robust_rule(float pid_sum,float FF, float error, float error_dot, float error_int, float delta_time)
{
	...
	float sign_s = 1;
	float rho = _intK0Yaw*FF + _intK1Yaw * error + _intK2Yaw * error_dot + _intK3Yaw * error_int;

    // Calculate rho
	float intK0_delta = (_eta*s*FF/(2025/4) - _asmc_alfa * _intK0Yaw) * delta_time;
	float intK1_delta = (_eta*s * error/(2025/4) - _asmc_alfa * _intK1Yaw) * delta_time;
	float intK2_delta = (_eta*s * error_dot/(2025/4) - _asmc_alfa * _intK2Yaw) * delta_time;
    float intK3_delta = (_eta*s * error_int/(2025/4) - _asmc_alfa * _intK3Yaw) * delta_time;
	if (_last_out < -45 || _last_out > 45) {
        ...
        float intK3_delta_temp = (_eta* - _asmc_alfa * _intK3Yaw) * delta_time;
        ...
        float _intK3Yaw_temp = _intK3Yaw; _intK3Yaw_temp += intK3_delta_temp;
		float rho_temp = _intK0Yaw_temp*FF + _intK1Yaw_temp * error + _intK2Yaw_temp * error_dot + _intK3Yaw_temp * error_int;
