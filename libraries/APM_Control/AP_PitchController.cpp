...
float AP_PitchController::_get_rate_out(float desired_rate, float scaler, bool disable_integrator, float aspeed, bool ground_mode)
{
	...
	//	float PID =  constrain_float(_last_out, -45, 45);
		float adaptive_robust_value = _update_pitch_adaptive_robust_rule(_last_out, pinfo.FF, pinfo.P, pinfo.D, pinfo.I, delta_time);
        if (_last_out < -45)
		{
 ...
 */
float AP_PitchController::_update_pitch_adaptive_robust_rule(float pid_sum, float FF, float error, float error_dot, float error_int, float delta_time)
{
    ...
    // Calculate rho
	float intK0_delta = (_eta*s*FF/(2025/4) - _asmc_alfa * _intK0Pitch) * delta_time;
	float intK1_delta = (_eta*s * error/(2025/4) - _asmc_alfa * _intK1Pitch) * delta_time;
	float intK2_delta = (_eta*s * error_dot/(2025/4) - _asmc_alfa * _intK2Pitch) * delta_time;
    float intK3_delta = (_eta*s * error_int/(2025/4) - _asmc_alfa * _intK3Pitch) * delta_time;
	if (_last_out < -45 || _last_out > 45) {
        ...
        float intK3_delta_temp = (_eta*  (- _asmc_alfa) * _intK3Pitch) * delta_time;
        ...
        float _intK3Pitch_temp = _intK3Pitch; _intK3Pitch_temp += intK3_delta_temp;
		float rho_temp = _intK0Pitch_temp*FF + _intK1Pitch_temp * error + _intK2Pitch_temp * error_dot + _intK3Pitch_temp * error_int;
