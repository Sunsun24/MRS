 ...
 */
float AP_TECS::_update_throttle_adaptive_robust_rule(float pid_sum, float FF,float error, float error_dot, float error_int)
{
	...
	float s       = pid_sum;  
	float sign_s = 1;
 	float rho = _intK0Thr*FF + _intK1Thr * error + _intK2Thr * error_dot + _intK3Thr * error_int;
	static float last_adapive_term = 0;
	float adaptive_term = 0;
	
	float intK0_delta = (_thrEta*s*FF - _asmc_thrAlfa * _intK0Thr) * _DT;
	float intK1_delta = (_thrEta*s * error - _asmc_thrAlfa * _intK1Thr) * _DT;
	float intK2_delta = (_thrEta*s * error_dot - _asmc_thrAlfa * _intK2Thr) * _DT;
    float intK3_delta = (_thrEta*s * error_int - _asmc_thrAlfa * _intK3Thr) * _DT;
	
	//throttle is beginning stop going down and ready to go up,  but we also should avoid K increase too much, to prevent overshoot
	if(_saturationFlag == 0)   
	{
		if(intK0_delta  > 0) {intK0_delta =  0;} 
		if(intK1_delta  > 0) {intK1_delta =   0;} 
		if(intK2_delta  > 0) {intK2_delta =   0;} 
        if(intK3_delta  > 0) {intK3_delta =   0;} 
	}
	else if (_saturationFlag == 1)   
	{
		if(intK0_delta  < 0) {intK0_delta = s * _DT*FF;}
		if(intK1_delta  < 0) {intK1_delta = s * error * _DT;}
		if(intK2_delta  < 0) {intK2_delta = s * error_dot * _DT;}
        if(intK3_delta  < 0) {intK3_delta = s * error_int * _DT;}
	}
	_intK0Thr += intK0_delta;  
	_intK1Thr += intK1_delta;
	_intK2Thr += intK2_delta;
    _intK3Thr += intK3_delta;
	if (_intK0Thr < -0.5) {
		_intK0Thr = -0.5;
	}
	if (_intK1Thr < -0.5) {
	...
	if (aparm.throttle_slewrate != 0) {
		...
		adaptive_term = constrain_float(adaptive_term,
										last_adapive_term - 0.5*thrRateIncr,
										last_adapive_term + 0.5*thrRateIncr);
		last_adapive_term = adaptive_term;
	}
	...
	//				(double)pid_sum, (double)error, (double)error_dot, (double)error_int ,  rho ,  _intK0Thr , _intK1Thr ,  _intK2Thr, sign_s, (double)s, (double)adaptive_term, _height,  pid_sum +adaptive_term, AP_HAL::millis());
...
void AP_TECS::_update_throttle_with_airspeed(void)
{
    ...
    if (_flags.underspeed) {
        ...
        const float maxAmp = 0.5f*(_THRmaxf - THRminf_clipped_to_zero   + _gainThr*last_adaptive_term);   //+ _gainThr*last_adaptive_term******
        const float integ_max = constrain_float((_THRmaxf - _throttle_dem + 0.1f),-maxAmp,maxAmp);  //- last_adaptive_term*************
        const float integ_min = constrain_float((_THRminf - _throttle_dem - 0.1f),-maxAmp,maxAmp);  //- last_adaptive_term*************
    ...
    // ***********Add adaptive-robust rule term***********************
		...
		if ( _switch != 0 ) //  && flag_takeoff == 1 )  //  && AP_HAL::millis() > 57320)
		{
			...
			if ( _switch == 1 )
			{
				...
				float adpative_robust_value = _update_throttle_adaptive_robust_rule(_throttle_dem, ff_throttle, e,  e_dot,  e_int);   
 ...
 */
float AP_TECS::_update_pitch_adaptive_robust_rule(float pid_sum, float FF, float error, float error_dot, float error_int)
{
	// Calculate the adaptive_robust_rule to better deal with the uncertainties.
	// tau = s + rho*sign(s); rho = K0 + K1*||xi|| + K2*||xi||^2; K_i_dot = ||s|| * ||xi||^i - alfa * K_i, i = 0,1,2;
	// xi = [error, error_dot, error_int];
	
	float s       = pid_sum;   // s = 1 + error +  error_dot +  error_int;
	//float norm_xi = sqrt(error*error + error_dot*error_dot + error_int*error_int);
	//float norm_s  = fabs(s);

	float sign_s = 1;
	float rho = _intK0Pitch*FF  + _intK1Pitch * error + _intK2Pitch * error_dot + _intK3Pitch * error_int;

    // Calculate rho
	float gainInv = (_TAS_state * timeConstant() * GRAVITY_MSS);
	
	float intK0_delta = (_pitchEta*s*FF  - _asmc_ptchAlfa * _intK0Pitch) * _DT;
	float intK1_delta = (_pitchEta*s * error - _asmc_ptchAlfa * _intK1Pitch) * _DT;
	float intK2_delta = (_pitchEta*s * error_dot - _asmc_ptchAlfa * _intK2Pitch) * _DT;
    float intK3_delta = (_pitchEta*s * error_int - _asmc_ptchAlfa * _intK3Pitch) * _DT;
    ...
    intK3_delta = constrain_float(intK3_delta, -integ_range*0.1f, integ_range*0.1f);
	...
	// float integK3_min = MIN(integ_min, _intK3Pitch);
	// float integK3_max = MAX(integ_max, _intK3Pitch);

    _intK0Pitch += intK0_delta;
	_intK1Pitch += intK1_delta;
	_intK2Pitch += intK2_delta;
    _intK3Pitch += intK3_delta;
    ...
    // _intK3Pitch = constrain_float(_intK3Pitch, integK3_min, integK3_max);
	if (_intK0Pitch < -0.5) {
	_intK0Pitch = -0.5;
	}
	if (_intK1Pitch < -0.5) {
...
void AP_TECS::_update_pitch(void)
{
    ...
    float SEBdot_dem = _hgt_rate_dem * GRAVITY_MSS * SPE_weighting + SEB_error / timeConstant();  //change position
    const float SEBdot_est = _SPEdot * SPE_weighting - _SKEdot * _SKE_weighting;  //change position
    ...
    // float SEBdot_dem = _hgt_rate_dem * GRAVITY_MSS * SPE_weighting + SEB_error / timeConstant();  // move it up
    ...
    //const float SEBdot_est = _SPEdot * SPE_weighting - _SKEdot * _SKE_weighting;   //move it up
    // float SEBdot_error = SEBdot_dem - SEBdot_est;
    ...
    float FF = (_TAS_dem_adj - _pitch_ff_v0) * _pitch_ff_k;    //***************
	...
	if (_switch != 0 )//   && flag_takeoff == 1 ) //  &&  AP_HAL::millis() > 57320)
	{
		...
		if(_switch == 1 )
		{
			//calculate adaptive robust term
				float adpative_robust_value = _update_pitch_adaptive_robust_rule(_pitch_dem,FF, e, e_dot, e_int);
