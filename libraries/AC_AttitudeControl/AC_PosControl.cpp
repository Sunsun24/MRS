...
void AC_PosControl::update_xy_controller()
{
    ...
    Vector2f vel_error;
    vel_error.x = _vel_target.x - curr_vel.x;
    vel_error.y = _vel_target.y - curr_vel.y;
    
    // acceleration to correct for velocity error and scale PID output to compensate for optical flow measurement induced EKF noise
    accel_target *= ahrsControlScaleXY;

	// get adaptive term******************
    //Vector2f vel_xy_adaptive_term;
	Vector2f vel_xy_adaptive_term(2,0);
	float adaptivescaler = 0.01;
/*	bool limit;
	...
	vel_xy_adaptive_term = _pid_vel_xy.get_adaptive_term(adaptivescaler);
    vel_xy_adaptive_term = vel_xy_adaptive_term;

//	printf("XY controller333333333333333333333333  original X   %f, original Y   %f,  adaptive term X %f,  adaptive term Y %f,  ekfNavVelGainScaler  %f\n", accel_target.x, accel_target.y, vel_xy_adaptive_term.x, vel_xy_adaptive_term.y, ekfNavVelGainScaler);
	accel_target.x += vel_xy_adaptive_term.x * ahrsControlScaleXY / adaptivescaler;  //adaptive
	accel_target.y += vel_xy_adaptive_term.y * ahrsControlScaleXY / adaptivescaler;  //adaptive 
...
// static float height[10000] = {0};
// static float ex[10000] = {0};
// static float ey[10000] = {0};
// static float epx[10000] = {0};
// static float epy[10000] = {0};
// //static float uroll[100000] = {0};
// //static float upitch[100000] = {0};
// static uint32_t i = 0;
// static uint32_t freq = 0;
// uint32_t k = 0;

// if(0==flag )    //  && (tnow>47000)
// {
// 	freq++;
// 	if( freq == 6)
// 	{
// 		height[i] = _inav.get_altitude() / 100.0;
// 		ex[i] = _vel_error.x;
// 		ey[i] = _vel_error.y;
// 		epx[i] = _pos_error.x;
// 		epy[i] = _pos_error.y;
// 	//	uroll[i] = _roll_target;
// 	//	upitch[i] = _pitch_target;
// 		i++;
// 		freq = 0;
// 	}
// }
// if(tnow > 150000 && 0 == flag)  //200000
// {
// 	flag++;
// 	printf("height_new = [...] and total  number is %d\n",i);
// 	for(k=0; k<i;k++)
// 	{
// 		 printf("%f ",height[k]);			
// 	}
// 	printf("\n");
// 	printf("ex = [...] and total  number is %d\n",i);
// 	for(k=0; k<i;k++)
// 	{
// 		 printf("%f ",ex[k]);			
// 	}
// 	printf("\n");
// 	printf("ey = [...] and total  number is %d\n",i);
// 	for(k=0; k<i;k++)
// 	{
// 		 printf("%f ",ey[k]);			
// 	}
// 	printf("\n");
// //	printf("target roll = [...] and total  number is %d\n",i);
// 	printf("epx = [...] and total  number is %d\n",i);
// 	for(k=0; k<i;k++)
// 	{
// 	//	 printf("%f ",uroll[k]);			
// 		printf("%f ",epx[k]);
// 	}
// 	printf("\n");
// //	printf("target pitch = [...] and total  number is %d\n",i);
// 	printf("epy = [...] and total  number is %d\n",i);
// 	for(k=0; k<i;k++)
// 	{
// 	//	 printf("%f ",upitch[k]);			
// 		printf("%f ",epy[k]);
...
void AC_PosControl::init_z_controller()
{
    ...
    // float error_z = _pos_target.z - curr_vel_z;
...
void AC_PosControl::update_z_controller()
{
    ...
    if (_vibe_comp_enabled) {
        ...
        float adaptive_term ;    //**************
        adaptive_term =  _pid_accel_z.get_adaptive_term(0.001);  //******************
//		printf("Z controller333333333333333333333333  original thr_out   %f,  adaptive term %f  \n", thr_out, adaptive_term);
        adaptive_term = adaptive_term; 
        thr_out += adaptive_term;    //adaptive 
        //thr_out = thr_out;             //PID
    }
