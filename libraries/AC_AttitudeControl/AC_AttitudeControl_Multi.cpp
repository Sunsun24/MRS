...
void AC_AttitudeControl_Multi::rate_controller_run()
{
	...
	adaptive_term = get_rate_roll_pid().get_adaptive_term(1);  
    _motors.set_roll(get_rate_roll_pid().update_all(_ang_vel_body.x, gyro_latest.x,  _dt, _motors.limit.roll, _pd_scale.x) + _actuator_sysid.x+ adaptive_term ); //
    _motors.set_roll_ff(get_rate_roll_pid().get_ff());
    
    adaptive_term = get_rate_pitch_pid().get_adaptive_term(1);  
    _motors.set_pitch(get_rate_pitch_pid().update_all(_ang_vel_body.y, gyro_latest.y,  _dt, _motors.limit.pitch, _pd_scale.y) + _actuator_sysid.y+ adaptive_term );  //
    _motors.set_pitch_ff(get_rate_pitch_pid().get_ff());
    
    adaptive_term = get_rate_yaw_pid().get_adaptive_term(1); 
    _motors.set_yaw(get_rate_yaw_pid().update_all(_ang_vel_body.z, gyro_latest.z,  _dt, _motors.limit.yaw, _pd_scale.z) + _actuator_sysid.z+ adaptive_term );  //
    _motors.set_yaw_ff(get_rate_yaw_pid().get_ff()*_feedforward_scalar);
    
 
    //*******error
	float throttle = _throttle_in;
    float eroll = get_rate_roll_pid().get_error();
	float epitch = get_rate_pitch_pid().get_error();
	float eyaw = get_rate_yaw_pid().get_error();
