...
public:
    ...
    Vector2f update_all3(const Vector3f &target, const Vector3f &measurement, float dt, const Vector3f &limit);
...
//****************
	...
	float _update_adaptive_robust_rule_x(float pid, float error, float error_dot, float error_int);
	float _update_adaptive_robust_rule_y(float pid, float error, float error_dot, float error_int);
	Vector2f get_adaptive_term(float scaler);
	
	float _alpha=1.0;//   
    float _eta= 10.0;
	float _eps= 10.0;
	float _integrator_scaler = 1.0f;
	uint32_t count = 0;
	double sum = 0;
	
	float _intK0_x = 0.00001;
	float _intK1_x = 0.00001;
	float _intK2_x = 0.00001;
    float _intK3_x = 0.00001;
	float _intK0_y = 0.00001;
	float _intK1_y = 0.00001;
	float _intK2_y = 0.00001;
    float _intK3_y = 0.00001;
//****************
