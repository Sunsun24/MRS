...
float AC_PID::_update_adaptive_robust_rule(float pid, float error, float error_dot, float error_int)
{
	...
	float sign_s = 1;
 	float rho = _intK0 + _intK1 * error + _intK2 * error_dot + _intK3 * error_int;
	float adaptive_term = 0;
//printf("_intK0 %f, _intK1%f, _intK2, intK3 %f \n" , _intK0, _intK1,  _intK2, intK3);
	if (is_positive(_dt))
	{
		if(!_shrink)
		{
			float intK0_delta =   (_eta*s - _alpha * _intK0) * _dt;
			float intK1_delta =   (_eta*s * error - _alpha * _intK1) * _dt;
			float intK2_delta =   (_eta*s * error_dot - _alpha * _intK2) * _dt;
            float intK3_delta =   (_eta*s * error_int- _alpha * _intK3) * _dt;
			_intK0 += intK0_delta;
			_intK1 += intK1_delta;
			_intK2 += intK2_delta;
            _intK3 += intK3_delta;
